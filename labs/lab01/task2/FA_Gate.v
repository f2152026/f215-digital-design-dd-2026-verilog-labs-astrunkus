...
module FA_Gate(
  ...
  wire ab_sum, ab_carry, cin_carry;

  half_adder_v3 lower (.a(a), .b(b), .sum(ab_sum), .carry(ab_carry));
  half_adder_v3 upper (.a(ab_sum), .b(cin), .sum(sum), .carry(cin_carry));
  or #(2) combine_carry(cout, ab_carry, cin_carry);
